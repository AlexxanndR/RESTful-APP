...
public class ParallelogramService {
    ...
    public OutputParams calculate(InputParams inputParams, CharacteristicType chrType) {
        ...
        if (chrType == CharacteristicType.perimeter) {
            ...
            outputParams.setPerimeter(perimeter);
   ...
   public <T> double calculateAverage(List<T> paramsList, ParamsType paramsType, CharacteristicType chrType) {
        double average = 0;
        if (!paramsList.isEmpty()) {
            MyLogger.log(Level.INFO, "Calculating average...");
            if (paramsType == ParamsType.input) {
            ...
            } else if (paramsType == ParamsType.output) {
                if (chrType == CharacteristicType.perimeter) {
                    average = paramsList.stream()
    ...
    public <T> double findMin(List<T> paramsList, ParamsType paramsType, CharacteristicType chrType) {
        double min = 0;
        if (!paramsList.isEmpty()) {
    ...
    public <T> double findMax(List<T> paramsList, ParamsType paramsType, CharacteristicType chrType) {
        double max = 0;
        if (!paramsList.isEmpty()) {
